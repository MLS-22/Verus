...
if cpu_model_line:
    ...
    if len(parts) >= 3:  # Ensure there are enough parts to extract numbers
        cpu_numbers = [part.split('-')[0].strip() for part in parts[1:]]

        # Check if there are enough cpu_numbers to form the URL
        if len(cpu_numbers) >= 2:
            ...
            print("Error: Not enough CPU model numbers.")
    else:
